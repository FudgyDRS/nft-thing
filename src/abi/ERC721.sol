/*
    ...
    tCRO: 0xaA7bb5855C88A5b95665BAadB99F04DeEe8E8de3
    Metadata of "rektapescontract" was published successfully.
    metadata.json : 
    dweb:/ipfs/QmaWfPEpV33nbYRG59FU7ptxR8zt25V1Ubs1WLaTVYeRUZ
...
contract RektApesContract is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    address public receiver = 0x0B3De431D3609C93c755747014C536763b378E33; //0x1C77905fEAfD85f7fdeD7aD5f871F93C2B11dcfF;

    constructor(string memory baseURI) ERC721("Rekt Apes", "Rekt Apes") {
        setBaseURI(baseURI);
        }
    ...
    function mintMultipleNFT(uint256 _amount) public payable noreentry {
        ...
        require(msg.value >= cost, "Value below mint price");
        currentSupply++;
        for (uint i = 0; i < _amount; i++) { _safeMint(msg.sender, 1 + currentSupply++); }
        }
    function calculatePrice() public view returns (uint256) {
        if      (currentSupply >= 801) { return 180 * 10**18; }
        else if (currentSupply >= 601) { return 160 * 10**18; }
        else if (currentSupply >= 401) { return 140 * 10**18; }
        else if (currentSupply >= 201) { return 120 * 10**18; }
        else                           { return 100 * 10**18; }
        }
    function calculatePriceMultiple(uint256 _amount) public view returns (uint256) {
        uint256 over = currentSupply + _amount;
        uint256 under;

        if      (over >= 801) { under = (801 - currentSupply) * 160; over = (over - 801) * 180; }
        else if (over >= 601) { under = (601 - currentSupply) * 140; over = (over - 601) * 160; }
        else if (over >= 401) { under = (401 - currentSupply) * 120; over = (over - 401) * 140; }
        else if (over >= 201) { under = (201 - currentSupply) * 100; over = (over - 201) * 120; }
        else                  { under = 0;                                over = _amount * 100; }
