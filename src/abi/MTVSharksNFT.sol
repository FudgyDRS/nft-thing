...
https://testnet.bscscan.com/address/0x3a3ac47e3E0E12de21295f30F14013C178d511DB#code

LIVE Contract:
0x5223A632FF5893975CB3d3aF73e8213d37a51D16
...

...
contract MTVSharksContract is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    constructor(string memory baseURI) ERC721("MTV Sharks", "MTV Sharks") {
        setBaseURI(baseURI);
        //for(uint i=0; i<25; i++) { _safeMint(owner(), i); }
        }
    ...
    function mintMultipleNFT(uint256 _amount) public payable {
        require(isSaleEnabled == true, "Sale hasn't started");
        ...
        require(msg.value >= cost, "MTV value below mint price");
        currentSupply++;
        for (uint i = 0; i < _amount; i++) { _safeMint(msg.sender, currentSupply++); }
        if(msg.value > cost) { returnOverpay(msg.value - cost); }
        }
    ...
    function calculatePriceMultiple(uint256 _amount) public view returns (uint256) {
        ...
        if      (over >= 2001) { under = (2001 - currentSupply) * 555; over = (over - 2001) * 666; }
        else if (over >= 1501) { under = (1502 - currentSupply) * 444; over = (over - 1501) * 555; }
